  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e) => {
      ...
      if (tool === 'pencil') {
        ...
        socket.emit('drawing', {
          boardId,
          element: { startX: offsetX, startY: offsetY, endX: offsetX, endY: offsetY, tool, color },
        });
      }
    };
    ...
    const handleMouseMove = (e) => {
      ...
      if (tool === 'pencil') {
        ...
        socket.emit('drawing', {
          boardId,
          element: { startX: startPosition.x, startY: startPosition.y, endX: offsetX, endY: offsetY, tool, color },
        });
        setStartPosition({ x: offsetX, y: offsetY });
      } else if (tool === 'eraser') {
        ...
        socket.emit('erase', {
          boardId,
          element: { x: offsetX, y: offsetY, tool },
        });
    ...
    <div className="flex flex-col min-h-screen w-full">
      <div className="flex p-4 bg-gray-800 text-white space-x-4 items-center">
        <button
          ...
          className={`p-2 rounded ${tool === 'pencil' ? 'bg-blue-500' : 'bg-gray-600'} hover:bg-blue-400`}
        >
          Pencil
        </button>
        <button
          onClick={() => setTool('eraser')}
          className={`p-2 rounded ${tool === 'eraser' ? 'bg-red-500' : 'bg-gray-600'} hover:bg-red-400`}
        >
          Eraser
        </button>
        <button
          onClick={() => setTool('rectangle')}
          className={`p-2 rounded ${tool === 'rectangle' ? 'bg-yellow-500' : 'bg-gray-600'} hover:bg-yellow-400`}
        >
          Rectangle
        </button>
        <button
          onClick={() => setTool('circle')}
          className={`p-2 rounded ${tool === 'circle' ? 'bg-green-500' : 'bg-gray-600'} hover:bg-green-400`}
        >
          Circle
        </button>
        <button
          onClick={() => setTool('text')}
          className={`p-2 rounded ${tool === 'text' ? 'bg-purple-500' : 'bg-gray-600'} hover:bg-purple-400`}
        >
          Text
        </button>
        <input
        ...
        <input
          ...
          className="p-2 border border-gray-600 rounded w-16"
          min="10"
        />
        <button
          onClick={() => navigate('/')}
